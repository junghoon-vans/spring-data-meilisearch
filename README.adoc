...
Requires Meilisearch 1.16 or later.
Directly paginating sorted document fetches through `MeilisearchClient.getRawDocuments(...)` is reliable from 1.27;
versions 1.16 through 1.26 may duplicate or omit documents between pages (see https://github.com/meilisearch/meilisearch/issues/5943[upstream issue #5943]).
Repository `findAll(Sort)` does not paginate document fetches.
