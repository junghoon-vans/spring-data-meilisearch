...
<.> The `stopWords` attribute defines stop words to ignore while searching.
====
...
* `*En` matches fields ending with `En`, such as `nameEn` and `titleEn`.
These fields are processed according to the language rules of the specified locales.
