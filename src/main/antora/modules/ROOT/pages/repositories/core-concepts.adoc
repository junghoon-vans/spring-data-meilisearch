...
* `findById(...)` uses direct document lookup; `findAllById(...)` uses native ID-list fetch.
* `findAll()` retrieves all documents in one request; sorted `findAll(Sort)` counts first, then fetches once.
* Paged `findAll(Pageable)` uses search APIs; an unpaged `Pageable` uses document listing and counts first if sorted.
* `count()` uses search APIs.
