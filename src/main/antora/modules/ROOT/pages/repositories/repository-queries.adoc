...
Repository interfaces that declare derived finder methods, repository `@Query` methods, or named-query-backed methods fail
during repository bootstrap with a Meilisearch-specific message.
This keeps unsupported query-method configuration from looking like an implemented feature.

[[meilisearch.repositories.queries.notes]]
== Practical Notes

* `findAll()` reads every document in one request through the document API; sorted `findAll(Sort)` counts first, then fetches once. Sorting requires a sortable attribute in the index settings.
* `findAllById(...)` uses native ID-list fetch, available from Meilisearch 1.14.
* Sorted `findAll(Sort)` and sorted unpaged `findAll(Pageable)` require Meilisearch 1.16.
* Paged `findAll(Pageable)` remains search-backed and inherits Meilisearch search pagination limits and search-only settings.
* An unpaged `Pageable` uses document-backed listing (with a preliminary count if sorted), so its content and total are not capped by `maxTotalHits`.
