...
* `findAll()` and `findAll(Sort)` read every document in batches through the document API; sorting requires a sortable attribute in the index settings.
* `findAllById(...)` uses native ID-list fetch, available from Meilisearch 1.14.
* Sorted `findAll(Sort)` requires Meilisearch 1.16; on 1.16 through 1.26, pagination across batches may duplicate or omit documents.
* `findAll(Pageable)` remains search-backed and inherits Meilisearch search pagination limits and search-only settings.
