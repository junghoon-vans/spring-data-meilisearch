...
The typical Meilisearch approach is to extend `MeilisearchRepository`, which combines Spring Data's blocking `CrudRepository` and `PagingAndSortingRepository` contracts for the mapped domain type.
`CrudRepository` gives you methods for CRUD functionality.
CRUD stands for Create, Read, Update, Delete.
`PagingAndSortingRepository` adds methods that accept a `Sort` or `Pageable` abstraction.

Spring Data Meilisearch currently supports the blocking repository model only.
Reactive, RxJava 3, and Kotlin coroutine repository variants such as `ReactiveCrudRepository`, `RxJava3CrudRepository`, `ReactiveSortingRepository`, `RxJava3SortingRepository`, `CoroutineCrudRepository`, and `CoroutineSortingRepository` are not supported by this module.
