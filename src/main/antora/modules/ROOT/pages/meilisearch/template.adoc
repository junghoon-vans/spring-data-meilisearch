...
Direct `get(...)` reads a known id. `findAll(...)` lists every document in batches,
then returns the complete result in memory; it does not return a single server-default-sized page.
