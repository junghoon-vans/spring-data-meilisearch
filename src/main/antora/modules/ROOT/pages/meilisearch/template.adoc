...
Direct `get(...)` reads a known id. `findAll(...)` requests every document at once and returns the complete result in memory;
it does not return a single server-default-sized page.
