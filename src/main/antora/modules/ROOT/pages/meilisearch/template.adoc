...
For multiple requested ids, `multiGet(...)` batches native ID-list reads on Meilisearch 1.14 and later.
The template checks the server version on the first such read and remembers the result for its lifetime.
Older servers, or API keys without the `version` action, continue to retrieve each id directly.
The returned entities retain the requested id order, excluding ids that do not exist.
