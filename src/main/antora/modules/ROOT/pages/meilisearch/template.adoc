...
For multiple requested ids, `multiGet(...)` batches native ID-list reads and returns entities in
requested id order, excluding ids that do not exist. The operations require Meilisearch 1.16 or later.
Document listing through `multiGet(Class<T>, int, int, Sort)` uses the document fetch API,
so unlike search results it is not limited by `@Pagination(maxTotalHits = ...)`.
