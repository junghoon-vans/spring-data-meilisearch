...
Direct `get(...)` reads a known id. Unsorted `findAll(...)` retrieves the documents in memory rather than a server-default-sized page.
Sorted listing counts documents first, then fetches up to that count in one request; documents added between the requests may not appear in the result.
