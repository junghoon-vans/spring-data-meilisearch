...
* `get(String, Class<T>)`
* `getDocuments(...)` for document listing
* `multiGet(...)` for ID-list retrieval
* `exists(String, Class<?>)`
* `delete(...)`
* `deleteAll(Class<?>)`

Direct `get(...)` reads a known id. `getDocuments(...)` lists documents without an
id filter: its no-argument overload returns one server-default-sized page, while
the offset/limit overloads request a specific page. `multiGet(...)` reads the
document ids supplied by the caller.
...
Document listing through `getDocuments(Class<T>, int, int, Sort)` uses server-side sorting available from
Meilisearch 1.16 and is not limited by `@Pagination(maxTotalHits = ...)`.
Sorted document pagination across batches is reliable from Meilisearch 1.27.
On 1.16 through 1.26, the server may duplicate or omit documents when sorting and paginating
