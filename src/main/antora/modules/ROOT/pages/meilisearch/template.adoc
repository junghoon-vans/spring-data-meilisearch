...
Direct `get(...)` reads a known id, while `multiGet(...)` also supports document listing and ID-list retrieval.
...
Document listing through `multiGet(Class<T>, int, int, Sort)` uses server-side sorting available from
